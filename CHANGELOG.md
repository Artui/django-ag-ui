...
- **The `pydantic-ai-slim[ag-ui]` floor is now `>=2.37`, and it buys a wire in
  which every `TOOL_CALL_START` names a message the stream announced.** A
  response whose first part is a tool call has no text to open its assistant
  message with. Below 2.37 the `parentMessageId` was emitted anyway and named a
  message no event in the stream carried -- a client could only answer by
  minting an id of its own, which then matches nothing in the history echoed
  back, so a replayed conversation cannot be told apart from new input. From
  2.37 an empty text message is opened and closed there instead.

  The checked-in wire fixture gained those two envelopes, which is why this is a
  floor rather than a lock bump: the fixture is generated by driving the real
  endpoint, the browser lane in another repository builds against it, and below
  2.37 it regenerates differently. Stated as a floor because a resolution that
  predates it produces a file this repository's own gate rejects.

  A new case asserts the property rather than leaving it to the byte
  comparison -- that test says the wire moved, this one says which way. It
  fails against the fixture as it stood before the bump, where both tool calls
  named messages nothing announced.

  Consumers rendering this stream should know that an empty text message can
  arrive, and that it is a declaration rather than an answer:
  `@artooi/ag-ui-web-component` drew a bubble for one until it was taught
  not to.
...
[Unreleased]: https://github.com/Artui/django-ag-ui/compare/v0.64.0...HEAD
