...
def build_untrusted_context(run_input: Any, *, config: RunContextConfig) -> str | None:
    ...
    Composes the two sources a browser client actually populates, in the order
    the model reads best: ``RunAgentInput.context`` first — the ambient
    situation, what page the user is on, what they have selected — then the
    attachment manifest, the specific handles the model can act on. Each source
    is a flag, so a project can take one without the other, and both reduce to
    :class:`~django_ag_ui.agent.types.untrusted_context_item.UntrustedContextItem`
    before rendering, so the fence treats them identically.

    ``context`` is a required field on ``RunAgentInput`` and typed
    ``list[Context]``, so there is nothing to guard against here — an entry is
    always a ``description`` / ``value`` pair. ``run_input`` is typed ``Any`` in
    keeping with the rest of this layer, where the AG-UI wire types arrive from
    pydantic-ai's adapter.
    """
