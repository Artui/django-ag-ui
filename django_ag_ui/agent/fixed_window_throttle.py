...
def _default_key(request: HttpRequest) -> str:
    ...
    The acting user is resolved by the time a throttle runs, so an authenticated
    caller is limited as themselves rather than sharing a bucket with everyone
...
class FixedWindowThrottle:
    ...
    Mirrors ``djangorestframework-mcp-server``'s ``FixedWindowRateLimit``, so the
    two transports behave the same way under the same configuration.
    """
    ...
    def consume(self, request: HttpRequest) -> int | None:
        ...
        # ``incr`` bumps it — the one fixed-window primitive that works on
        # Memcached, Redis and locmem alike. A read-then-write would lose counts
        # to the very concurrency this exists for.
    ...
    def _increment(self, cache_key: str) -> int:
        ...
        ``incr`` raises ``ValueError`` when the key is gone — an eviction, or the
        window expiring since ``add`` reported it present — so re-seed: the
        window has no surviving record, making this the first call in it.
        """
        try:
