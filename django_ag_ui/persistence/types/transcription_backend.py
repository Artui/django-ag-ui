...
class TranscriptionBackend(Protocol):
    ...
    no-op default ([`NullTranscriptionBackend`][django_ag_ui.NullTranscriptionBackend]
    — voice off)
    and an opt-in reference implementation over an OpenAI-compatible
    ``/audio/transcriptions`` endpoint
    ([`OpenAITranscriptionBackend`][django_ag_ui.contrib.transcription.openai_transcription_backend.OpenAITranscriptionBackend]).
