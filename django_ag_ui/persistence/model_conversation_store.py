...
class ModelConversationStore(ABC):
    ...
    async def list(self, *, request: HttpRequest) -> ConversationMetaList:
        return await sync_to_async(self._list_scoped)(request)
    ...
    def _list_scoped(self, request: HttpRequest) -> ConversationMetaList:
        return self._list(resolve_owner_id(request))
